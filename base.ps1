...
Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Search" -Name "BackgroundAppGlobalToggle" -Value 0 -Type DWord
...
# Disable Microsoft Edge from startup programs
$edgeStartupPath = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run"
if (Test-Path $edgeStartupPath) {
    Remove-ItemProperty -Path $edgeStartupPath -Name "MicrosoftEdge" -ErrorAction SilentlyContinue
}

# Remove Microsoft Edge shortcut from the Start Menu
$edgeShortcutPath = "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Microsoft Edge.lnk"
if (Test-Path $edgeShortcutPath) {
    Remove-Item $edgeShortcutPath -ErrorAction SilentlyContinue
}

# Install Thorium Browser
$thoriumInstallerUrl = (Invoke-RestMethod -Uri "https://api.github.com/repos/Alex313031/Thorium-Win/releases/latest").assets | Where-Object { $_.name -like "*Installer.exe" } | Select-Object -ExpandProperty browser_download_url
$installerPath = "$env:TEMP\ThoriumInstaller.exe"
Invoke-WebRequest -Uri $thoriumInstallerUrl -OutFile $installerPath
Start-Process -FilePath $installerPath -ArgumentList "/silent" -Wait

# Set Thorium Browser as the default browser
$thoriumAppPath = "C:\Program Files\Thorium\Thorium.exe"
$defaultBrowserCommand = "start ms-settings:defaultapps"
Start-Process -FilePath "cmd.exe" -ArgumentList "/c $defaultBrowserCommand" -Wait
Start-Process -FilePath $thoriumAppPath

# Pin Thorium Browser to the taskbar
$taskbarPath = "$env:APPDATA\Microsoft\Internet Explorer\Quick Launch\User Pinned\TaskBar"
$thoriumShortcutPath = "$taskbarPath\Thorium.lnk"
if (-not (Test-Path $thoriumShortcutPath)) {
    $shell = New-Object -ComObject WScript.Shell
    $shortcut = $shell.CreateShortcut($thoriumShortcutPath)
    $shortcut.TargetPath = $thoriumAppPath
    $shortcut.Save()
}

# Install PowerShell 7 (PowerShell Core)
$pwshInstallerUrl = (Invoke-RestMethod -Uri "https://api.github.com/repos/PowerShell/PowerShell/releases/latest").assets | Where-Object { $_.name -like "*win-x64.msi" } | Select-Object -ExpandProperty browser_download_url
$installerPath = "$env:TEMP\pwshInstaller.msi"
